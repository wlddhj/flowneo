...
describe('buildSessionContext', () => {
  it('有绑定：Router + 本任务 status', () => {
    ...
    expect(ctx).toContain('<FLOWNEO_SESSION>')
    expect(ctx).toContain('session_id: s1')
    expect(ctx).toContain('bound: a')
  ...
  it('无绑定：Router + 任务列表', () => {
    ...
    setupTask('a', '1')
    setupTask('b', '4')
    const ctx = buildSessionContext(dir, undefined, 's-none')
    expect(ctx).toContain('<FLOWNEO_SESSION>')
    expect(ctx).toContain('session_id: s-none')
